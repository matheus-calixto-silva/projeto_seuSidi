...
const MeuPonto = () => {
  ...
  const handlePrev = () => {
    setActiveIndex((prevIndex) => (prevIndex - 1 + dates.length) % dates.length);
  };
    ...
    <div className={style.pageContainer}>
      <div className={style.mainContent}>
        ...
        <div className={style.carousel}>
          <button onClick={handlePrev} className={style.carouselButton} aria-label="Anterior">&lt;</button>
          <span>{dates[activeIndex]}</span>
          <button onClick={handleNext} className={style.carouselButton} aria-label="Próximo">&gt;</button>
        </div>
        ...
        <section className={style.content}>
          <article className={style.timeRecords}>
            ...
            <table className={style.table}>
              ...
              <tbody>
                <tr>
                  <td>25/10 Sexta</td>
                  <td>Início: 10:50 <br />Fim: 18:00</td>
                  <td><span role="img" aria-label="atencao">⚠️</span> Aguardando processamento</td>
                  <td>
                    <button onClick={handleRegister} className={style.registerButton}>Registrar</button>
                    <button onClick={handleSuggestion} className={style.suggestButton}>Sugerir ajuste</button>
                  </td>
                </tr>
                <tr>
                  <td>24/10 Quinta</td>
                  <td>Início: 09:00 <br />Fim: 17:00</td>
                  <td>Falta</td>
                  <td>
                    <button onClick={handleRegister} className={style.registerButton}>Registrar</button>
                    <button onClick={handleSuggestion} className={style.suggestButton}>Sugerir ajuste</button>
          ...
          <article className={style.summary}>
            ...
            <div className={style.totalHoursBox}>
              <p className={style.totalHours}>Horas em folha: <span className={style.greenText}>180h</span></p>
            </div>
